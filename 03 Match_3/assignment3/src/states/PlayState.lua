...
--[[
    Calculates whether there are any possible moves to make matches.
]]
function PlayState:ableMoves()

    local ableMoves = 0

    self.boardHighlightY2 = self.boardHighlightY
    self.boardHighlightX2 = self.boardHighlightX

    for y = 1, 8 do

        self.highlightedTile = self.board.tiles[y][1] 
        for x = 1 , 7 do
            self.highlightedTile = self.board.tiles[y][x]
            self.boardHighlightX = math.min(7, self.boardHighlightX + 1)
    ...
    for x = 1, 8 do
        ...
        self.highlightedTile = self.board.tiles[1][x] 
        for y = 1, 7 do
            self.highlightedTile = self.board.tiles[y][x]
            self.boardHighlightY = math.min(7, self.boardHighlightY + 1)
    ...
    self.highlightedTile = nil
    self.boardHighlightY = self.boardHighlightY2
    self.boardHighlightX = self.boardHighlightX2
end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            self.score = self.score + self.level / 2 * (#match * 12) + #match * 25
