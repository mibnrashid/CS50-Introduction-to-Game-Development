...
function PlayState:enter(params)
    ...
    self.ball2 = Ball2()
    self.locked_brick = Locked_Brick()
    self.locked_brick.num = math.random(0,1)

    self.powerup = Powerup()
    if self.locked_brick.num == 1 then
        self.powerup.skin = 10
    else
        self.powerup.skin = math.random(7,9)
    end
...
function PlayState:update(dt)
    ...
    if self.powerup:collides(self.paddle) and self.locked_brick.num == 0 then
        self.ball2.inPlay = true
    elseif self.powerup:collides(self.paddle) and self.locked_brick.num == 1 then
        self.powerup.lock = false
    end

    if self.ball:collides(self.locked_brick) and self.powerup.lock == false then

        self.score = self.score + 2000

        self.locked_brick.num = 0
