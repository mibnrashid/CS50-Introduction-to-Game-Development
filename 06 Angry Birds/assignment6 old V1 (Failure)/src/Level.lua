...
function Level:init()
    ...
    local xVel2, yVel2 = 1, 0
    local xVel3, yVel3 = 1, 0
    self.vel2 = math.abs(xVel2) + math.abs(yVel2)
    self.vel3 = math.abs(xVel3) + math.abs(yVel3)
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        if love.keyboard.wasPressed('space') and self.minialiens == 0 and self.hit == false then
            self.alien2 = Alien(self.world, 'round', self.launchMarker.alien.body:getX(), self.launchMarker.alien.body:getY() + 5, 'Player')
            table.insert(self.players, self.alien2)
            self.alien2.fixture:setRestitution(0.4)
            self.alien2.body:setAngularDamping(1)
            self.alien2.body:setLinearVelocity((self.launchMarker.baseX - self.launchMarker.shiftedX) * 10, (self.launchMarker.baseY - self.launchMarker.shiftedY) * 10)
            self.alien3 = Alien(self.world, 'round', self.launchMarker.alien.body:getX(), self.launchMarker.alien.body:getY() - 5, 'Player')
            table.insert(self.players, self.alien3)
            self.alien3.fixture:setRestitution(0.4)
            self.alien3.body:setAngularDamping(1)
            self.alien3.body:setLinearVelocity((self.launchMarker.baseX - self.launchMarker.shiftedX) * 10, (self.launchMarker.baseY - self.launchMarker.shiftedY) * 10)
            self.minialiens = 2
        elseif love.keyboard.wasPressed('space') and self.minialiens == 2 then
            xVel2, yVel2 = self.alien2.body:getLinearVelocity()
            xVel3, yVel3 = self.alien3.body:getLinearVelocity()
            self.vel2 = math.abs(xVel2) + math.abs(yVel2)
            self.vel3 = math.abs(xVel3) + math.abs(yVel3)

            if (math.abs(xVel) + math.abs(yVel) < 1.5) and self.vel2 < 1.5 and self.vel3 < 1.5 then
